...
このプロジェクトは、従業員の日々の勤務状況を記録し、月次の作業報告書や勤怠管理表として簡単に管理・印刷するためのWebアプリケーションです。手作業による時間計算や報告書作成の煩雑さを解消し、正確かつ効率的な勤怠管理を実現することを目的としています。

バックエンドはPythonの軽量フレームワークであるFlask、フロントエンドはモダンなJavaScriptライブラリのReactを使用して構築されており、シンプルで直感的に操作できるUIを提供します。

## 主な機能

- **作業報告書の作成**: ExcelライクなUIで、日付、出退勤時刻、休憩時間、作業内容を直感的に入力できます。
- **勤怠管理表**: 作業報告書とは別に、勤怠種別（出勤、欠勤、有給など）や深夜休憩などを記録でき、より詳細な勤怠管理が可能です。
- **自動計算機能**: 勤務時間、休憩時間、深夜休憩などを入力すると、実働時間、残業時間、深夜労働時間などが自動で計算・集計されます。
- **日報機能**: 日々の詳細な業務内容や課題、翌日の予定などを記録し、月ごとに一覧で確認できます。
- **承認機能**: 管理者（オーナー）は、提出された勤務表を承認または承認取り消しできます。承認日は自動で記録されます。
- **マスターメンテナンス**: 管理者権限を持つユーザーが、社員情報の追加や更新を行えます。
- **印刷プレビュー**: 作成した作業報告書や勤怠管理表を、A4用紙に最適化された印刷用レイアウトでプレビュー・印刷できます。
...
1.  **社員の選択**: トップページに表示される社員一覧から、自分の名前を選択します。
2.  **勤務表の入力**:
    -   年月を選択すると、該当月の勤務表が表示されます。
    -   各日付の欄をクリックし、出勤・退勤時間、休憩時間、作業内容を入力します。
    -   入力が完了したら、「保存」ボタンをクリックしてデータを保存します。
3.  **日報の確認**: 「日報」ボタンをクリックすると、その月の詳細な業務内容を一覧で確認できます。
4.  **印刷**: 「印刷」ボタンをクリックすると、印刷プレビューが表示され、作業報告書を印刷できます。
5.  **マスターメンテナンス（管理者向け）**:
    -   オーナーとして設定された社員（`back_end/num.id`で指定）は、「マスターメンテナンス」ボタンが表示されます。
    -   この画面から、社員情報の追加や更新が可能です。
...
本プロジェクトへの貢献に興味をお持ちいただき、ありがとうございます。バグ報告、機能提案、プルリクエストなど、あらゆる形の貢献を歓迎します。

貢献に関心のある方は、Issueを作成するか、プルリクエストを送信してください。
